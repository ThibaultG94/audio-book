...
const [selectedVoice, setSelectedVoice] = useState("voices/fr/fr_FR/siwis/low/fr_FR-siwis-low.onnx");
...
const getVoiceDisplayName = (voice: VoiceModel) => {
  if (voice.language) {
    ...
    return `${voice.language.name_native} - ${voice.dataset}${quality}`;
  }
  return voice.name;
};
...
const getPresetSamples = () => {
  return {
    "audiobook_comfort": "Voici un extrait de roman lu de manière confortable pour une écoute prolongée.",
    "news_efficient": "Voici un bulletin d'information lu de façon claire et efficace.",
    "storytelling_dramatic": "Il était une fois, dans un royaume lointain, une princesse aux pouvoirs magiques...",
...
return (
  <div className={`bg-white rounded-xl border border-gray-200 shadow-sm p-6 ${className}`}>
      ...
      <div className="mt-2 flex items-center justify-between">
        {availableVoices.length === 0 ? (
          <p className="text-xs text-amber-600">
            ⚠️ Seule la voix par défaut est installée
          </p>
        ) : (
          <p className="text-xs text-green-600">
            ✅ {availableVoices.length} voix disponible{availableVoices.length > 1 ? 's' : ''}
          </p>
        )}
        
        <button
          onClick={() => window.open('https://github.com/rhasspy/piper/releases', '_blank')}
          className="text-xs text-blue-600 hover:text-blue-800 underline"
        >
          + Installer plus de voix
        </button>
      </div>
    </div>
    ...
    {/* Preview Text Input - IMPROVED STYLING */}
    <div className="mb-6">
      <div className="flex items-center justify-between mb-2">
        <label className="block text-sm font-medium text-gray-700">
          Texte d'aperçu
      ...
      <textarea
        ...
        style={{ 
          fontSize: '15px',
          lineHeight: '1.6',
          color: '#111827', // Much darker text - gray-900
          fontWeight: '400'
        }}
      />
    ...
    <div className="flex items-center justify-between">
      <div className="flex items-center space-x-3">
        <button
          ...
          className="flex items-center px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium"
        >
          {isGenerating ? (
    ...
    {/* Voice count indicator with installation guide */}
      <div className="flex items-center space-x-4">
        <div className="text-xs text-gray-500">
          {availableVoices.length > 0 ? (
            <span>{availableVoices.length} voix disponible{availableVoices.length > 1 ? 's' : ''}</span>
          ) : (
            <span>Voix par défaut uniquement</span>
          )}
        </div>
        
        {availableVoices.length <= 1 && (
          <div className="text-xs">
            <span className="text-amber-600">💡 Plus de voix ?</span>
            <button
              onClick={() => window.open('https://github.com/rhasspy/piper/releases', '_blank')}
